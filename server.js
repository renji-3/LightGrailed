...
app.get("/messages", (req, res) => {
  ...
  return db.query(`SELECT messages.*, products.*, users.* FROM messages JOIN products ON products.id = messages.product_id JOIN users ON users.id = messages.user_id WHERE users.id = $1 OR products.seller_id = $2 ORDER BY messages.id DESC`, [user_id, user_id])
    .then((response) => {
      console.log("response:", response.rows);
      const templateVars = {
        product: response.rows,
